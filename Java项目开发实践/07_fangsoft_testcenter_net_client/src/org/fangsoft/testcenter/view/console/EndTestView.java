...
public class EndTestView extends ConsoleView implements TestCenterView {
	...
	protected void displayView() {
		Console.output("考试结束，现在时间是：" + Configuration.getDateFormat().format(tr.getEndTime()));
		long duration = 0;
		if (timeout) {
		...
		Console.output("=============== 考试报告 ===============");
		Console.output("%1$-6s%2$-6s%3$-6s%4$-6s%n", "题号", "你的答案", "正确答案", "对错");
		int count = 0;
		...
		String displayPass = "";
		if (tr.getResult().getIntVal() == 1) {
			displayPass = "恭喜，你通过了考试";
		}
		else if (tr.getResult().getIntVal() == -1) {
			displayPass = "Unknow";
		}
		else {
			displayPass = "很遗憾，你没有通过考试";
		}
		Console.output("你考试的得分是：" + tr.getScore() + ", " + displayPass);
	}
}
